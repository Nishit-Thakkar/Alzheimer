...
if os.path.exists('my_model_weights.h5'):
    ...
    weight_url = "https://drive.google.com/file/d/1VE4-ffyr86S6sPP09EmIYGGqMJWkOQXd/view?usp=sharing"
    gdown.download(weight_url, 'my_model_weights.h5', quiet=False,fuzzy=True)
if os.path.exists('model_arch.json'):
    pass
else:
    json_url = "https://drive.google.com/file/d/1v7f4NghK5VlLANlPWlB-vLywGYiDr3Lt/view?usp=sharing"
...
html_temp = """
    ...
    <h2 style="color:white;text-align:center;">Alzheimer Detection</h2>
    <p style="color:white;text-align:center;" >This is a <b>Streamlit</b> app use for prediction of the <b>4 Stage of Alzheimer</b>.</p>
    </div>
    """
...
if option == 'Choose your own image':
    ...
    if uploaded_file is not None:
        ...
        img = image.load_img(uploaded_file, target_size=(128,128,3))
        pred_class = predict_img(img)
        col1, col2 = st.columns(2)
        with col1:
            st.image(img, width=200)
        with col2:
            st.success("Alzheimer Type:  [" + str(pred_class) + "] ")
        form()
else:
